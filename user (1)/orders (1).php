...
if($status==$s1)
{
    $query="select * from tbl_ordermaster where  del_status='$s1' and cust_id='$user' order by o_date";
    $tbl=getDatas($query);
}
    else if($status==$s2)
{
     $query="select * from tbl_ordermaster where del_status='$s2'  and cust_id='$user'order by o_date";
    $tbl=getDatas($query);
}
...
{
     $query="select * from tbl_ordermaster where  del_status='$s3'  and cust_id='$user' order by o_date";
    $tbl=getDatas($query);
}
...
{
     $query="select * from tbl_ordermaster where del_status='$s4' and cust_id='$user' order by o_date";
       $tbl=getDatas($query);
}
